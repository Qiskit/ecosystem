...
class ShellCommand(Command):
    ...
    @classmethod
    def execute(cls, command: List[str], directory: str, **kwargs) -> CommandExecutionSummary:
        return cls.subprocess_execute(command=command,
                                      cwd=directory,
                                      name=" ".join(command))
...
class CloneRepoCommand(Command):
    ...
    def execute(cls, repo: str, directory: str, **kwargs) -> CommandExecutionSummary:
        return cls.subprocess_execute(["git", "-C", directory, "clone", repo],
                                      name="Clone repo")
...
class RunToxCommand(Command):
    ...
    def execute(cls, directory: str, env: str, **kwargs) -> CommandExecutionSummary:
        return cls.subprocess_execute(["tox", "-e{}".format(env), "--workdir", directory],
                                      cwd=directory,
                                      name="Tests")
    ...
    def execute(cls, file: str, directory: str, **kwargs) -> CommandExecutionSummary:
        if not os.path.isfile("{}/{}".format(directory, file)):
            return CommandExecutionSummary(code=1,
                                           logs=[],
                                           summary="No {} file in project.".format(file))
