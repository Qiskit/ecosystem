...
This documentation site is built with [ProperDocs](https://properdocs.org) (the drop-in
...
- `website/index.html`, an HTML file that redirects to the ecosystem page. This is needed because we
  used to host the ecosystem page on GitHub pages, so it overwrites the index that ProperDocs generates.
